...
## Что это?
 ...
 Обратимся к [Книге Творения](/docs/literature/sefer_yetzirah-burmistrov/):
...
Видим, описана такая же полярная структура: есть `"Горячий полюс"` - **`ШИН`**, `"Холодный полюс"` - **`МЕМ`** и еще нечто ``"среднее"``, некое наполнение структуры - среда, изначально нейтральная, но "управляемая" полюсами. Это **`Алеф`** - _"преклоняется между огнем и водой"_ (между полюсами). т.е. "поляризуется", управляется ими.
...
Первое, что важно: **ШИН** - отвечающая за "Горячий полюс" - идет под номером **21**.

Почему в треугольнике **"Горячий полюс"** - **"Алеф"** - **"Холодный полюс"** - не **БЕТ** (номер 2)? Почему не **ТАУ** (номер 22)? Тому есть объяснение:

Дело в том, что в рамках "высокополяризованного" языка, каким является иврит, а его "поляризация" как раз под знаком "Горячего полюса", **"Двойка"** возведена в абсолют. Это, посуществу, высшее божество. Главная, непререкаемая истина и величина. Поэтому в треугольнике **ШИН** - **АЛЕФ** - **МЕМ** **"Двойка"** участвует не напрямую, а в виде **21**-ой буквы **ШИН**, как бы, одной из своих эманаций. Однако, это не далает ее слабее. В определенном смысле, напротив, **"Двойка"** "управляющая" силой **"Единицы"** (**21**) получает дополнительные преферентии, если можно так выразиться.

Аналогичные рассуждения касаются и **МЕМ**. Ведь **МЕМ** не девятая. Она тринадцатая. И тут мы имеем дело с еще одним проявлением "поляризации" иврита, как метафизической системы. А именно, девятая буква **ТЕТ**, как бы попадает под запрет: не употреблять лишний раз имя "врага". Совершенно неслучайно единственная буква, которой нет на **Скрижалях Завета**, дарованных Богом Моисею, - это девятая буква - буква **ТЭТ**.

И здесь, в связке **ШИН** - **АЛЕФ** - **МЕМ**, вместо **ТЕТ** фигурирует тринадцатая - **МЕМ**.

Остановимся на причинах этого подробней:
